...
public class MainLoop {
     ...
     * <em>returning</em> {@code true}, which is easy to read backwards and easier still to drop, so a
     * test that times out carries on to its assertions and then fails somewhere else with no
     * indication that the wait was the problem. This throws instead, naming what was awaited and how
     * long it ran.
     * <pre><code>
     *     mainLoop.awaitUntil("the delayed action to fire", () -&gt; probe.fired);
     * </code></pre>
     *
